...
import SubMenu from '@/components/SubMenu/monitoring/SubMenu.vue'
...
export default {
  ...
  methods: {
    ...
    onClickBreadCrumb() {
      ...
      this.$router.push({ name: 'networkMonitor', params: { id: netIdLocal } })
    },
    async getData() {
  ...
  watch: {
    $route: {
      ...
      handler() {
        ...
        if (this.path.name === 'networkMonitor') {
          this.getData()
